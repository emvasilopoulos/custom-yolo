...
class RadialDecayMode(enum.Enum):
    GAUSSIAN = enum.auto()
    LINEAR = enum.auto()


class BaseYOLOLossPerFeatureMapV3(torch.nn.Module):
    def __init__(
        self,
        num_classes: int,
        ...
        super(BaseYOLOLossPerFeatureMapV3, self).__init__()
    ...
    def define_radial_decay_mode(
        ...
        """
        Define the radial decay mode for the loss function.
        Args:
            radial_decay_mode (str): The radial decay mode to be used.
        """
        pass
...
def _build_feature_map_targets(
    ...
    epsilon: float = 1e-4,
    radial_decay_mode: RadialDecayMode = RadialDecayMode.GAUSSIAN,
    ...
    gy_top = (g_y - (sel_bh * grid_size_h).int() // 2).clamp(min=0)
    gy_bottom = (g_y + (sel_bh * grid_size_h).int() // 2).clamp(max=grid_size_h - 1)
    gx_left = (g_x - (sel_bw * grid_size_w).int() // 2).clamp(min=0)
    gx_right = (g_x + (sel_bw * grid_size_w).int() // 2).clamp(max=grid_size_w - 1)
    ...
    for i in range(len(a_idx)):
        ...
        decrease_h = -1 if H % 2 == 0 else 0
        decrease_w = -1 if W % 2 == 0 else 0
        if H + decrease_h <= 2 or W + decrease_w <= 2:
            # We want at least 3x3 area for radial decay otherwise 1x1 is enough (covered by the center objectness)
            continue

        # TODO: set sigma according to grid size
        radial_decay_map = radial_decay(
            H + decrease_h,
            W + decrease_w,
            mode=radial_decay_mode,  # with linear I believe it won't work that well
            sigma=None,
            dtype=targets.dtype,
            device=targets.device,
        )

        # don't let radial decay map to update higher scores
        targets[
            a_idx[i],
            4,
            gy_top[i] : gy_bottom[i] + decrease_h,
            gx_left[i] : gx_right[i] + decrease_w,
        ] = torch.maximum(  # element-wise max
            targets[
                a_idx[i],
                4,
                gy_top[i] : gy_bottom[i] + decrease_h,
...
def radial_decay(
    H: int,
    W: int,
    mode: RadialDecayMode = RadialDecayMode.LINEAR,
